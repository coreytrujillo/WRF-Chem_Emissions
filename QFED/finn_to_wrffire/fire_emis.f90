...
!-----------------------------------------------------------------
   ...
   use fire_file, only : write_fire_file, max_fire_size
...
!-------------------------------------------------------------------------
!psp     read(unit=10,'(a)',iostat=istat) buffer
...
!-------------------------------------------------------------------------
     ...
     rewind(10)
     if( has_fire_size(file) ) then
!psp       read(unit=10,*,iostat=istat) dummy,dummy,dummy,dummy,dummy,dummy,fire_species(1:n_fire_spc)
       read(10,*,iostat=istat) dummy,dummy,dummy,dummy,dummy,dummy,fire_species(1:n_fire_spc)
     else
!psp       read(unit=10,*,iostat=istat) dummy,dummy,dummy,dummy,dummy,fire_species(1:n_fire_spc)
       read(10,*,iostat=istat) dummy,dummy,dummy,dummy,dummy,fire_species(1:n_fire_spc)
     endif
     if( istat /= 0 ) then
...
!psp       read(unit=10,*,iostat=istat) day(n,file)
       read(10,*,iostat=istat) day(n,file)
       if( istat /= 0 ) then
...
!psp       read(unit=10,*,iostat=istat) buffer
       read(10,*,iostat=istat) buffer
       if( istat /= 0 ) then
         write(*,*) 'fire_emis: failed to read ',trim(filespec),'; error = ',istat
         stop 'Read error'
       endif
       do n = 1,beg_day_ndx(1)-1
!psp         read(unit=10,*,iostat=istat) timeod
         read(10,*,iostat=istat) timeod
         if( istat /= 0 ) then
           write(*,*) 'fire_emis: failed to read ',trim(filespec),'; error = ',istat
           stop 'Read error'
         endif
...
day_loop : &
       do day_ndx = 1,day_cnt-1
         ...
         do m = m1,m2
           if( has_fire_size(file) ) then
!psp             read(unit=10,*,iostat=istat) timeod,timeod,genveg,lat(m),lon(m),fire_size(m),fire_emissions(:,m)
             read(10,*,iostat=istat) timeod,timeod,genveg,lat(m),lon(m),fire_size(m),fire_emissions(:,m)
           else
!psp             read(unit=10,*,iostat=istat) timeod,timeod,genveg,lat(m),lon(m),fire_emissions(:,m)
             read(10,*,iostat=istat) timeod,timeod,genveg,lat(m),lon(m),fire_emissions(:,m)
...
!          write(*,'(''fire_emis: calling write_glb @ day_ndx,m1,m2 = '',i6,2i8)') day_ndx,m1,m2
           call write_glb_fire_file( domain, n_wrf_spc, m1, m2, lon_ndx, &
